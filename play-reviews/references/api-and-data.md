...
| 文件 | 角色 |
|------|------|
| `<skill目录>/scripts/serve_reviews.py` | 本地服务：静态托管 `assets/` + `GET /api/scrape` 抓取接口；页面「拉取最新」按钮调它。看板/数据路径全部由脚本自身位置解析，任意目录启动均可 |
| `<skill目录>/scripts/scrape_play_reviews.py` | CLI 抓取（产出 CSV）；`T1/T2/T3_MARKETS` 与 `MARKET_TIERS` 定义市场层级，增删市场改这里 |
...
- 请求：`GET /api/scrape?app=<包名>[&tier=<t1|t2|t3>][&count=<每国上限，默认 3000>]`
- `tier` 市场层级（累积语义，默认 `t1`）：
...
- 响应：NDJSON 流，逐行 JSON：
  ...
  - `{"type":"done","payload":{"app_id","title","generated_at","reviews":[…]}}` 结束（`title` 为 Play 商店应用名，拉取失败时退回包名）
...
- payload/apps JSON：`{ app_id, title, source_csv, generated_at, reviews:[同 CSV 列的 JSON] }`
- data.js：`window.REVIEWS_APPS = { apps:[{app_id, title, total, generated_at, data:<同 payload>}] }`（内联多包镜像）

## 备用抓取（服务器起不来时）

```bash
python3 <skill目录>/scripts/scrape_play_reviews.py <包名> 3000 [--tier t2]   # → <skill目录>/assets/csv/<app>.csv, 默认 tier=t1
python3 <skill目录>/scripts/csv_to_reviews_js.py <上述csv> <skill目录>/assets/dashboard/data.js --app <包名>
```
...
- **范围边界**：仅支持 Android（Google Play）；不含 iOS App Store。命令链（`lsof`/`ps`/`nohup`/`curl`）为 macOS/Linux 语法，Windows 需 WSL 或等效命令。
...
- **网络前提**：抓取需可访问 play.google.com；国内网络通常需要代理，否则全部国家报错、服务器返回 fatal。
...
- **评论池按语言切分**：同语言国家返回完全相同的评论集合（如 t1 的 us/gb/ca/au，t2 里 at/ch 与 de 也同池），页面以 ≈ 标注同池；区分地区靠 `lang` 而非 `country`。
