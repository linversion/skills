...
def fetch_title(app_id: str) -> str:
    """从 Play 商店拉应用展示名, 失败则退回包名。"""
    try:
...
def scrape_to_payload(app_id: str, per_country: int, markets: dict, emit) -> dict:
    ...
    # 全部国家都失败 => 网络层问题(如无法访问 play.google.com), 判整体失败而非落盘 0 条
    if not merged and errors:
        raise RuntimeError(
            f"全部 {len(markets)} 个国家抓取失败, 疑似网络问题(需可访问 play.google.com, "
...
def write_data_js() -> None:
    ...
    for e in index:
        try:
            ...
            continue
        entry = {k: e.get(k) for k in ("app_id", "title", "total", "generated_at")}
        entry["data"] = data
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def handle_scrape(self):
        ...
        print(f"[scrape] {app_id} tier={tier} ({len(markets)} 国, 每国上限 {per_country})", flush=True)
        ...
        try:
            emit({"type": "start", "tier": tier, "markets": len(markets)})
            payload = scrape_to_payload(app_id, per_country, markets, emit)
            payload["title"] = fetch_title(app_id)
